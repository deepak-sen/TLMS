...
class User {	
	...
	public function getUsersListing(){		
		...
		$sqlQuery = "
			SELECT id, first_name, last_name, sanket, designation, section, email, type, deleted
			FROM ".$this->userTable."  
			$whereQuery ";
		...
		while ($user = $result->fetch_assoc()) { 				
			...
			if($user['type'] == 1){
				...
				$type = '<span class="label label-warning">Emp.</span>';
			}
	...
	public function insert(){
		...
		if($this->email && $this->password) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->userTable."(`first_name`, `last_name`,`sanket`,`designation`,`section`, `email`, `password`, `type`, `deleted`)
				VALUES(?,?,?,?,?,?,?,?,?)");
		
			$this->first_name = htmlspecialchars(strip_tags($this->first_name));
			$this->last_name = htmlspecialchars(strip_tags($this->last_name));
			$this->sanket = htmlspecialchars(strip_tags($this->sanket));
			...
			$stmt->bind_param("sssssssii", $this->first_name, $this->last_name,$this->sanket,$this->designation,$this->section, $this->email, md5($this->password), $this->type, $this->deleted);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET first_name= ?, last_name = ?,sanket = ?,designation = ?,section = ?, email = ?, type = ?, deleted= ?
				WHERE id = ?");
			...
			$stmt->bind_param("sssssssii", $this->first_name, $this->last_name,$this->sanket,$this->designation,$this->section, $this->email, $this->type, $this->deleted, $this->id);
