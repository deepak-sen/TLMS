...
class Post {	
	...
	public function getPostsListing(){		
		...
		$sqlQuery = "
			SELECT p.id, p.title,p.message, p.pdate, p.category_id, u.first_name, u.last_name, p.status, p.created, p.updated, c.name 
		...
		while ($post = $result->fetch_assoc()) { 				
			...
			$rows[] = '<a href="edit_post.php?id='.$post["id"].'" class="btn btn-warning btn-xs update">Edit</a>';
	...
	public function getPost(){		
		if($this->id) {
			$sqlQuery = "
				SELECT p.id, p.title, p.message, p.pdate, p.category_id, p.status, p.created, p.updated, c.name 
				FROM ".$this->postTable." p
				LEFT JOIN ".$this->categoryTable." c ON c.id = p.category_id
				WHERE p.id = ? ";
			$stmt = $this->conn->prepare($sqlQuery);
	...
	public function insert(){
		...
		if($this->title && $this->message) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->postTable."(`title`, `message`, `pdate`, `category_id`, `userid`, `status`, `created` , `updated`)
				VALUES(?,?,?,?,?,?,?,?)");
			...
			$stmt->bind_param("sssiisss", $this->title, $this->message, $this->pdate, $this->category, $this->userid, $this->status, $this->created, $this->updated);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET title= ?, message = ?, pdate = ?,category_id = ?, status= ?, updated = ?
				WHERE id = ?");
			...
			$stmt->bind_param("sssissi", $this->title, $this->message, $this->pdate, $this->category, $this->status, $this->updated, $this->id);
