...
class User {	
	...
	public function getUsersListing(){		
		...
		$sqlQuery = "
			SELECT id, first_name, last_name, email, designation,section, type, deleted
			FROM ".$this->userTable."  
			$whereQuery ";
		...
		while ($user = $result->fetch_assoc()) { 				
			...
			if($user['type'] == 1){
				...
				$type = '<span class="label label-warning">Employee</span>';
			}
			
			$rows[] = ucfirst($user['first_name'])." ".$user['last_name'];
			$rows[] = $user['email'];
			$rows[] = $user['designation'];
			$rows[] = $user['section'];
			$rows[] = $type;			
			$rows[] = $status;				
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET first_name= ?, last_name = ?, email = ?, type = ?, password = ?, deleted= ?
				WHERE id = ?");
			...
			$stmt->bind_param("sssssii", $this->first_name, $this->last_name, $this->email, $this->type, md5($this->password), $this->deleted, $this->id);
